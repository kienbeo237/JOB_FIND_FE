  ...
  return (
    <tr className="border-b hover:bg-gray-50">
      ...
      <td className="px-4 py-3">
        ...
        {order.status === "cancelled" && <Badge className="bg-red-100 text-red-800 border-red-200">Bị huỷ</Badge>}
        {order.status === "pending" && (
          <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200">Chờ xử lý</Badge>
        )}
        {order.status === "processing" && (
          <Badge className="bg-blue-100 text-blue-800 border-blue-200">Đang xử lý</Badge>
        )}
      </td>
      <td className="px-4 py-3">
        <div>
          ...
          <p
            className={`text-sm ${
              new Date(order.expirationDate.split("/").reverse().join("-")) < new Date()
                ? "text-red-600"
                : "text-green-600"
            }`}
          >
      ...
      <td className="px-4 py-3">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => onViewDetails(order)}>Xem chi tiết</DropdownMenuItem>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center">
        <Button variant="ghost" className="flex items-center text-gray-600" onClick={onBack}>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="border-l-4 border-green-500 pl-4 py-2 bg-green-50 mb-6">
          <div className="grid grid-cols-3 gap-6">
            ...
            <div>
              ...
              <p className="font-medium">{order.paymentDate || "Chưa thanh toán"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className={`font-medium ${order.paymentAmount ? "text-green-600" : "text-red-600"}`}>
                {order.paymentAmount
                  ? `${new Intl.NumberFormat("vi-VN").format(order.paymentAmount)} VND`
                  : "Chưa thanh toán"}
              </p>
            </div>
          </div>
        ...
        <h3 className="font-bold text-gray-800 uppercase mb-4">THÔNG TIN ĐƠN HÀNG</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 text-left">
                ...
                <th className="px-4 py-3 text-gray-600 text-center">Số lượng</th>
        ...
        <div className="mt-6 space-y-2 text-right">
          ...
          <div className="flex justify-end text-xs text-gray-500">
            <span className="w-48">*Giá trị chiết khấu của mỗi dịch vụ tối đa 50% giá niêm yết</span>
            <span className="w-32"></span>
          </div>
          <div className="flex justify-end">
            <span className="w-48 text-gray-600">Tổng tiền chưa bao gồm VAT:</span>
          ...
          <div className="flex justify-end pt-2 border-t mt-2">
            <span className="w-48 text-gray-600 font-bold">Tổng số tiền thanh toán:</span>
            <span className="w-32 font-bold text-green-600">11,456,640 VND</span>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <p className="text-sm text-gray-600 mb-1">Tên công ty / Hộ kinh doanh:</p>
            <p className="font-medium">CÔNG TY TNHH MILENSEA RETAIL</p>
          </div>
          <div className="md:col-span-2">
            <p className="text-sm text-gray-600 mb-1">Địa chỉ công ty:</p>
            <p className="font-medium">
              Phòng 1701, Tầng 17 Khu văn phòng, Tòa nhà Pearl Plaza, 561A Điện Biên Phủ, Phường 25, Quận Bình Thạnh,
...
export default function OrdersPage() {
  ...
  const filterOrders = () => {
    ...
    if (activeTab === "processing") return orders.filter((order) => order.status === "processing")
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-medium text-gray-600">Tạo đơn hàng</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-600">Giá trị đơn hàng</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-600">Trạng thái thanh toán</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-600">Trạng thái đơn hàng</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-600">Kích hoạt dịch vụ</th>
