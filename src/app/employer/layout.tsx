...
function EmployerLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const { activeTab, setActiveTab, activeSettingsTab, setActiveSettingsTab } = useDashboard()
  ...
  const handleNavItemClick = (title: string, href: string, hasDropdown?: boolean) => {
    if (hasDropdown) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-16">
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center">
              <span className="text-xl md:text-2xl font-bold text-green-600">JobFind.vn</span>
            </Link>

            {/* Navigation */}
            <nav className="hidden md:flex items-center space-x-6">
              <Link href="/employer/jobs" className="text-gray-700 hover:text-green-600 transition-colors">
                Giới thiệu
              </Link>
              <button
                onClick={() => {
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 bg-white shadow-lg rounded-lg p-2 w-56 z-50 border border-gray-100">
                    <div className="border-b pb-2 mb-2">
                      ...
                      <p className="text-sm text-gray-500 px-3">hr@company-abc.com</p>
                    </div>
                    <Link
                      href="/employer/dashboard"
          ...
          {mobileMenuOpen && (
            <div className="md:hidden py-4 bg-white shadow-lg rounded-lg mt-2">
              <div className="space-y-2 px-4">
                <Link href="/employer/jobs" className="block text-gray-700 hover:text-green-600 py-2 transition-colors">
                  Giới thiệu
                </Link>
                <button
                  onClick={() => {
                ...
                <div className="border-t border-gray-100 my-2 pt-2">
                  <div className="flex items-center justify-between py-2">
                    ...
                    <span className="bg-blue-100 text-blue-600 text-xs px-2 py-1 rounded">NTD</span>
                  </div>
                  <Link
                    href="/employer/dashboard"
      ...
      <div className="container mx-auto py-6 px-4">
        {isAuthPage ? (
          <div className="bg-white rounded-lg overflow-hidden shadow-sm">{children}</div>
        ) : (
          <div className="flex rounded-lg overflow-hidden shadow-sm">
            <div className="w-64 min-w-[240px] bg-white flex-shrink-0">
              ...
              <div className="h-[calc(100vh-10rem)] overflow-y-auto py-4">
                <nav className="px-2">
                  {navItems.map((item) => {
                    const isActive =
                      (activeTab === "dashboard" && item.title === "Tổng quan") ||
                      (activeTab === "settings" && item.title === "Cài đặt tài khoản") ||
                      (activeTab === "packages" && item.title === "Các gói & mua") ||
                      (activeTab === "cart" && item.title === "Giỏ hàng & thanh toán") ||
                      (activeTab === "orders" && item.title === "Theo dõi đơn hàng") ||
                      (activeTab === "services" && item.title === "Dịch vụ còn hiệu lực") ||
                      (activeTab === "jobs" && item.title === "Quản lý tin đăng") ||
                      (activeTab === "candidates" && item.title === "Quản lý CV ứng viên") ||
                      (activeTab === "cv-labels" && item.title === "Quản lý nhãn CV") ||
                      (activeTab === "messages" && item.title === "Tin nhắn") ||
                      (activeTab === "activity" && item.title === "Lịch sử hoạt động") ||
                      (activeTab === "support" && item.title === "Chuyên viên CSKH")
                    ...
                    return (
                      <div key={item.href} className="mb-1">
                        {item.hasDropdown ? (
                          <>
                            <button
                              onClick={() => handleNavItemClick(item.title, item.href, item.hasDropdown)}
                            ...
                            >
                              <div className="flex items-center">
                                <item.icon
                                  className={`mr-2 h-5 w-5 ${isActive ? "text-green-600" : "text-gray-500"}`}
                                />
                            ...
                            {isOpen && (
                              <div className="mt-1 ml-4 pl-4 border-l border-gray-200 w-full">
                                {item.dropdownItems?.map((subItem) => {
                                  const isSubActive =
                                    activeTab === "settings" &&
                                    ((subItem.title === "Người dùng đăng nhập" && activeSettingsTab === "user-login") ||
                                      (subItem.title === "Danh sách người dùng" && activeSettingsTab === "user-list") ||
                                      (subItem.title === "Tạo người dùng phụ" && activeSettingsTab === "create-user") ||
                                      (subItem.title === "Thông tin công ty" && activeSettingsTab === "company-info") ||
                                      (subItem.title === "Giấy phép kinh doanh" &&
                                        activeSettingsTab === "business-license") ||
                                      (subItem.title === "Đổi mật khẩu" && activeSettingsTab === "change-password") ||
                                      (subItem.title === "Dịch vụ API" && activeSettingsTab === "api-service"))

                                  return (
                                    <button
                                      key={subItem.href}
                                      onClick={() => handleSettingsItemClick(subItem)}
                          ...
                          <button
                            onClick={() => (item.onClick ? item.onClick() : handleNavItemClick(item.title, item.href))}
                          ...
                          >
                            <item.icon className={`mr-2 h-5 w-5 ${isActive ? "text-green-600" : "text-gray-500"}`} />
                            <span>{item.title}</span>
                          </button>
                        )}
                      </div>
            ...
            <div className="flex-1 bg-white">
              {activeTab === "post-job" ? (
                ...
                <div className="p-6">
                  <h1 className="text-xl font-medium mb-6 text-teal-700">Lựa chọn dịch vụ đăng banner còn hiệu lực</h1>
                  ...
                  <Card className="mb-6">
                    <CardContent className="pt-6">
                              ...
                              <tr key={service.id} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                                <td className="px-4 py-4">
                                  <div className="flex flex-col">
                                    ...
                                    <a href={service.previewLink} className="text-sm text-green-600 hover:underline">
                                      Xem mẫu tả
                                    </a>
                                  </div>
                                </td>
                                <td className="px-4 py-4 text-center">{service.currentAmount}</td>
                                <td className="px-4 py-4 text-center">
                                  <RadioGroup defaultValue={index === 0 ? service.id : ""}>
                                ...
                                <td className="px-4 py-4 text-center">{service.remainingAmount}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      <div className="mt-10">
                        <h2 className="text-lg font-medium mb-4">Tải Banner lên để duyệt</h2>
                        <div className="bg-gray-100 border border-gray-200 rounded-md p-4 mb-6">
                          <p className="text-gray-700">File Png, Jpeg dưới 5 MB</p>
                        </div>

                        <div className="flex space-x-4">
                          <Button variant="outline" className="border-teal-600 text-teal-600 hover:bg-teal-50 w-1/3">
                            Xem trước
                          </Button>
                          <Button className="bg-teal-600 hover:bg-teal-700 w-2/3">Gửi đề xuất</Button>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <div className="bg-teal-600 text-white p-6 rounded-md">
                    <h3 className="font-medium text-lg mb-2">Gói dịch vụ đăng banner trang chủ 10 ngày</h3>
                    <p>
                      Hiển thị banner quảng cáo trên trang chủ với vị trí đẹp, thu hút nhiều lượt xem từ ứng viên tiềm
                      năng.
                    </p>
                  </div>
                </div>
