...
// Custom tooltip for the time chart
const TimeChartTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 border border-gray-200 shadow-md rounded-sm">
        <p className="text-gray-700 font-medium">{label}</p>
        <p className="text-violet-600">total : {payload[0].value}</p>
      </div>
    );
  }
  return null;
...
// Custom tooltip for the job position chart
const JobChartTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 border border-gray-200 shadow-md rounded-sm">
        <p className="text-gray-700 font-medium">{payload[0].payload.name}</p>
        <p className="text-green-600">value : {payload[0].value}</p>
      </div>
    );
  }
  return null;
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    ...
                    width={100}
                    tickFormatter={value => {
                      // Format the job titles to match the image
