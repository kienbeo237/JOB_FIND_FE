...
export function BannerAdvertising({
  ...
  const [activeBannerPackage, setActiveBannerPackage] = useState<string>("banner-homepage")
  ...
  const handleAddToCart = useCallback(
    (packageName: string, period: string, price: number, quantity: number) => {
      ...
      const existingCart = JSON.parse(localStorage.getItem("bannerCart") || "[]")
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-800">Dịch Vụ Quảng Cáo Banner</h2>
          ...
          activeBannerPackage === key && (
            <div key={key} className="border rounded-lg overflow-hidden shadow-sm">
              ...
              <div className={`bg-gradient-to-r ${pkg.gradient} p-6 text-white`}>
                ...
                <p className="text-white/80 mt-1">Quảng cáo hiệu quả, tiếp cận đúng ứng viên</p>
              </div>
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="w-full">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Thời hạn</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {pkg.pricing.map((price) => (
                          <tr key={price.period} className="bg-white">
                            <td className="px-4 py-4 text-sm text-gray-900">{price.period}</td>
                            <td className="px-4 py-4 text-sm text-gray-900">{formatPrice(price.price)} VND</td>
                            <td className="px-4 py-4 text-sm text-gray-900">
                              <div className="flex items-center space-x-2">
                                <Button
                                  ...
                                  size="icon"
                                  className="h-8 w-8"
                                  onClick={() => handleQuantityChange(key, price.period, -1)}
                                  disabled={!quantities[key]?.[price.period] || quantities[key]?.[price.period] <= 0}
                                >
                                  <Minus className="h-4 w-4" />
                                </Button>
                                <span className="w-8 text-center">{quantities[key]?.[price.period] || 0}</span>
                                <Button
                                  variant="outline"
                                  size="icon"
                                  className="h-8 w-8"
                            ...
                            <td className="px-4 py-4 text-sm font-medium text-gray-900">
                              {formatPrice(calculateTotal(price.price, quantities[key]?.[price.period] || 0))} VND
                            </td>
                            <td className="px-4 py-4 text-sm text-gray-900">
                              <Button
