...
export function JobPostingServices({
  ...
  const handleAddToCart = (packageName: string, period: string, price: number, quantity: number) => {
    ...
    const existingCart = JSON.parse(localStorage.getItem("jobPostingCart") || "[]")
  ...
  const packageData = {
    ...
    premiumPlus: {
      title: "Gói PREMIUM+",
      description: "Gói bán chạy nhất (Gói nâng PREMIUM+), nhiều doanh nghiệp lựa chọn",
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Gói Dịch Vụ Đăng Tin</h2>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-gray-50">
                        ...
                        <th className="py-2 px-3 text-left text-sm font-medium text-gray-700 border-b">Giá (VND)</th>
                    ...
                    <tbody>
                      {pkg.pricing.map((price) => (
                        <tr key={price.period} className="border-b">
                          ...
                          <td className="py-3 px-3 text-sm">{formatPrice(price.price)} VND</td>
                          <td className="py-3 px-3">
                            <div className="flex items-center space-x-2">
                              <Button
                                ...
                                disabled={!(quantities[key]?.[price.period] || 0)}
                              >
