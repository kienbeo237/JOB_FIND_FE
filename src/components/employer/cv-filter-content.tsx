  ...
  const [candidates, setCandidates] = useState([
    {
      ...
      cvDetails: {
        ...
        certificates: ["Google Digital Marketing Certificate (2022)", "Facebook Blueprint Certification (2023)"],
        languages: [
    ...
    if (selectedCandidateId) {
      setCandidates(
        candidates.map((candidate) =>
          candidate.id === selectedCandidateId ? { ...candidate, contactUnlocked: true } : candidate,
        ),
      )
    ...
    <div className="flex-1 bg-gray-50 p-6">
      <Card className="mb-6">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            ...
            <Button className="bg-orange-500 hover:bg-orange-600">Tìm kiếm</Button>
          </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-3">
                    <div className="w-full">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 mt-2">
                        <div className="space-y-2">
                          <p className="text-sm text-gray-700 flex items-center gap-2">
                            ...
                            <span className="font-medium text-gray-800 min-w-16">Kinh nghiệm:</span>
                            <span className="text-gray-600">{candidate.experience}</span>
                          </p>
                          <p className="text-sm text-gray-700 flex items-center gap-2">
                            <GraduationCap className="w-4 h-4 text-gray-500" />
                            <span className="font-medium text-gray-800 min-w-16">Học vấn:</span>
                            <span className="text-gray-600">{candidate.education}</span>
                          </p>
                          <p className="text-sm text-gray-700 flex items-center gap-2">
                            <MapPin className="w-4 h-4 text-gray-500" />
                            <span className="font-medium text-gray-800 min-w-16">Địa điểm:</span>
                        ...
                        <div className="space-y-2 mt-1.5 sm:mt-0">
                          <p className="text-sm text-gray-700 flex items-start gap-2">
                            ...
                            <span className="font-medium text-gray-800 min-w-16 mt-0.5">Kỹ năng:</span>
                          ...
                          <p className="text-sm font-medium text-gray-700 flex items-center gap-2">
                            ...
                            <span className="font-medium text-gray-800 min-w-16">Trạng thái:</span>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100">
                        <p className="text-sm text-gray-600 line-clamp-2">{candidate.cvDetails.summary}</p>
                      </div>
                        ...
                        <div className="flex items-center text-sm">
                          ...
                          <span className="text-gray-500">
                            {candidate.contactUnlocked ? (
                              <span>
                                Email:{" "}
                                <span className="text-blue-600">{candidate.email.replace("***", "uyenvien")}</span>
                              </span>
                            ) : (
                              <span>
                                Email: <span className="text-gray-400">{candidate.email}</span>
                              </span>
                            )}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center text-sm">
                          ...
                          <span className="text-gray-500">
                            {candidate.contactUnlocked ? (
                              <span>
                                SĐT:{" "}
                                <span className="text-blue-600">{candidate.phone.replace("******", "222333")}</span>
                              </span>
                            ) : (
                              <span>
                                SĐT: <span className="text-gray-400">{candidate.phone}</span>
                              </span>
                            )}
                          </span>
                        </div>
        ...
        <div className="flex justify-center mt-8">
          <nav className="flex items-center space-x-1">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-gray-100">394</button>
            <button className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-gray-100">395</button>
            <button className="px-2 py-1 flex items-center justify-center rounded-md hover:bg-gray-100">&gt;</button>
          </nav>
        </div>
      </div>
      ...
      <Dialog open={showCVDialog} onOpenChange={setShowCVDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Thông tin liên hệ đã được ẩn đi. Mở khóa thông tin để xem thông tin đầy đủ.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            <div className="border-b pb-6">
              <div className="flex items-start gap-6">
                ...
                <div>
                  ...
                  <p className="text-lg text-gray-600">
                    ...
                    {selectedCandidate?.skills && selectedCandidate.skills.includes("UI/UX") ? "Designer" : "Developer"}
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                    <div className="flex items-center text-sm text-gray-600">
                      {selectedCandidate?.contactUnlocked ? (
                        <span>
                          Email:{" "}
                          <span className="text-blue-600">{selectedCandidate?.email.replace("***", "uyenvien")}</span>
                        </span>
                      ) : (
                        <div className="flex items-center">
                          <span>
                            Email: <span className="text-gray-400">{selectedCandidate?.email}</span>
                          </span>
                    ...
                    <div className="flex items-center text-sm text-gray-600">
                      {selectedCandidate?.contactUnlocked ? (
                        <span>
                          SĐT:{" "}
                          <span className="text-blue-600">{selectedCandidate?.phone.replace("******", "222333")}</span>
                        </span>
                      ) : (
                        <div className="flex items-center">
                          <span>
                            SĐT: <span className="text-gray-400">{selectedCandidate?.phone}</span>
                          </span>
                          <Lock className="h-4 w-4 ml-2 text-gray-400" />
                        </div>
                      )}
              ...
              <div className="mt-6">
                ...
                <p className="mt-2 text-gray-600">{selectedCandidate?.cvDetails.summary}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-5">
                {selectedCandidate?.cvDetails.workExperience.map((exp, index) => (
            ...
            {selectedCandidate?.cvDetails.certificates && selectedCandidate.cvDetails.certificates.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                ...
                <ul className="list-disc pl-5 space-y-1">
                  {selectedCandidate.cvDetails.certificates.map((cert, index) => (
            ...
            {selectedCandidate?.cvDetails.languages && selectedCandidate.cvDetails.languages.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {selectedCandidate.cvDetails.languages.map((lang, index) => (
                    <div key={index} className="flex items-center">
                      <span className="min-w-32 text-gray-700">{lang.name}:</span>
            ...
            {selectedCandidate?.cvDetails.projects && selectedCandidate.cvDetails.projects.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {selectedCandidate.cvDetails.projects.map((project, index) => (
                    <div key={index} className="border-l-2 border-gray-200 pl-4">
      ...
      <Dialog open={showPaywallDialog} onOpenChange={setShowPaywallDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Bạn sẽ dùng 1 credit để mở khóa thông tin liên hệ của ứng viên này</DialogDescription>
          </DialogHeader>
            ...
            <div className="border rounded-lg p-4 bg-gray-50">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    setShowPaywallDialog(false)
                    // Redirect to packages page in a real app
                  }}
                >
                  Mua thêm
                </Button>
              ...
              <ul className="mt-2 space-y-2">
                <li className="flex items-center text-sm text-gray-600">
                  ...
                  <span className="text-gray-900 font-medium">
                    {selectedCandidate?.email.replace("***", "uyenvien@gmail.com")}
                  </span>
                </li>
                <li className="flex items-center text-sm text-gray-600">
          ...
          <DialogFooter className="sm:justify-between">
            ...
            <Button type="button" className="bg-blue-600 hover:bg-blue-700" onClick={confirmUnlockContact}>
      ...
      <Dialog open={showVideoDialog} onOpenChange={setShowVideoDialog}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Video giới thiệu của {selectedCandidate?.name}</DialogTitle>
            <DialogDescription>Xem video giới thiệu của ứng viên</DialogDescription>
          </DialogHeader>
          ...
          <div className="aspect-video bg-black rounded-lg overflow-hidden">
            ...
            <div className="h-full w-full flex flex-col items-center justify-center text-white p-6">
              ...
              <p className="text-center mb-4">Video giới thiệu của {selectedCandidate?.name}</p>
              <p className="text-sm text-center text-gray-400">Video này đang được mô phỏng cho mục đích trình diễn</p>
      ...
      <Dialog open={showAudioDialog} onOpenChange={setShowAudioDialog}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Audio giới thiệu của {selectedCandidate?.name}</DialogTitle>
            <DialogDescription>Nghe audio giới thiệu của ứng viên</DialogDescription>
          </DialogHeader>
          ...
          <div className="bg-gray-100 rounded-lg p-6">
            ...
            <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200">
              <h4 className="text-sm font-medium text-gray-700 mb-2">Nội dung giới thiệu</h4>
              <p className="text-sm text-gray-600">
                "Xin chào, tôi là {selectedCandidate?.name}. Tôi có {selectedCandidate?.experience} kinh nghiệm làm việc
                trong lĩnh vực {selectedCandidate?.skills && selectedCandidate.skills[0]}. Tôi đã tốt nghiệp{" "}
                {selectedCandidate?.education} và đang tìm kiếm cơ hội mới để phát triển sự nghiệp..."
              </p>
            </div>
          </div>
