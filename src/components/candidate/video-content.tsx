...
const VideoContent = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>(hasVideo ? "view" : "create")
  ...
  const startRecording = async () => {
    try {
      ...
      setUploadError("Không thể truy cập camera. Vui lòng kiểm tra quyền truy cập camera của bạn.")
    }
  }
    ...
    <div className="container mx-auto py-8 px-4 md:px-6">
      <Card className="max-w-2xl mx-auto">
        <CardContent className="p-6">
          <Tabs defaultValue={hasVideo ? "view" : "create"} value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="create">
              ...
              <p className="text-gray-600 mb-6 text-sm">
                Video giới thiệu bản thân ngắn gọn sẽ giúp nhà tuyển dụng hiểu rõ hơn về bạn và tăng cơ hội được chọn
                vào vòng phỏng vấn. Hãy chuẩn bị một không gian yên tĩnh và trang phục phù hợp trước khi quay video.
              </p>
              ...
              {recording ? (
                    ...
                    <div className="absolute bottom-4 left-0 right-0 flex justify-center">
                      <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full flex items-center gap-2">
                        ...
                        <span className="text-white text-sm font-medium">
                          {recordingTime < 10 ? `0:0${recordingTime}` : `0:${recordingTime}`}
                          /0:30
                        </span>
                      </div>
                    </div>
                ...
                <div className="flex justify-center gap-4 mt-6">
                  <Button className="bg-blue-100 text-blue-700 hover:bg-blue-200 min-w-32" onClick={startRecording}>
                    Bắt đầu quay
                  </Button>
              ...
              {previewVideo && !recording && (
                <div className="mt-6 space-y-4">
                  <div className="bg-black aspect-video rounded-lg overflow-hidden">
                    <video src={previewVideo} controls className="w-full h-full" />
                  </div>
              ...
              {uploadProgress > 0 && uploadProgress < 100 && (
                <div className="mt-6">
                  ...
                  <p className="text-sm text-gray-600 text-center">Đang tải lên... {uploadProgress}%</p>
                </div>
              )}

              {uploadError && <div className="mt-4 p-3 bg-red-50 text-red-600 rounded-md text-sm">{uploadError}</div>}
            </TabsContent>

            <TabsContent value="view">
              <div className="space-y-6">
                <div className="bg-black aspect-video rounded-lg overflow-hidden">
                  <video src={savedVideo || undefined} controls className="w-full h-full" />
                </div>
                <div className="flex justify-between">
                  <Button
                    variant="outline"
